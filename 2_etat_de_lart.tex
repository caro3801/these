...
déclaratives (\gls{SVG}) et impératives (canvas) sont issues de la normalisation du HTML5, alors 
...
sujet ou type d'évènement dans la couche réseau et construisent et maintiennent la 
...
changements 
de l'état d'une application\fg{}'.  \og [une] série d'évènements [qui] capture tout ce qui est 
...
(dans 
notre exemple, les informations sont inhérentes aux évènements). De plus, les 
...
du temps (linéaire), considérant que les commandes ont parfois besoin de connaître une partie de 
 l'état courant issu de ces évènements pour être déclenchées.
