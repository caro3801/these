...
la scène vierge) lié à la scène où chaque micro-version stocke uniquement les 
informations relatives à son parent : deux n\oe uds peuvent être issus du même 
parent menant à la création d'une nouvelle branche, i.e. nouvel espace de travail 
séparé.
...
exemple, le calcul de deux rotations consécutives se traduit par l'opération 
non-commutative
de la multiplication de deux matrices \gls{3D}.
...
un avantage certain pour les système critiques ou les applications collaboratives. 
La pratique la plus 
...
\end{enumerate*}
L'origine de ces changements identifiée, le chef de projet peut intervenir 
...
les événements étant stockés depuis le début de la vie -- mise en route -- de 
l'application, beaucoup plus de données peuvent être analysées. Par 
...
l'utilisation de l'\gls{ES}. En effet, pour récupérer l'état courant à partir de l'Event 
Store, il est nécessaire de le calculer à partir des événements reçus.
...
aussi. Cependant, il doit être évité pour ne pas créer de dépendance entre 
...
Le déterminisme est appliqué en assurant à l'événement toutes les 
