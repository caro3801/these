...
Les événements issus du journal d'événements (ou Event Store), en passant 
dans la partie \og 
lecture\fg{} du modèle \gls{CQRS} peuvent être consommés par différentes 
projections accrochées au bus d'événement. De cette manière, une fois le journal 
d'événements synchronisé, les projections locales sont identiques (à une 
cohérence éventuelle près). 
Un journal d'événements est composé d'agrégats versionnés qui contiennent les 
