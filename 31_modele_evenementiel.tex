...
storageAPI), pour l'affichage \gls{3D} (WebGL) et pour la communication en 
\gls{P2P} 
...
transmission des données \gls{3D} et collaboratives qui permet de  limiter  le 
nombre 
...
d'objets \gls{3D} mais aussi au domaine de la collaboration. Par exemple le terme 
de 
...
et la visualisation d'objets \gls{3D} dans un \gls{EVC}. La détection d'événement 
est un 
...
Le modèle orienté événement pour la modélisation \gls{3D} collaborative présenté 
dans 
