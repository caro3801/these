...
fine granularité. Selon le scenario suivant par exemple $UserA$ a effectué une 
une translation $MeshTrans\-latedEvent$ sur le maillage 
$mesh1$ lorsque $UserB$ a effectué une rotation $MeshRotatedEvent$ sur ce 
même objet. Lorsque les événements sont sauvegardés, la version de l'agrégat 
$mesh1$ est différente chez chacun des deux utilisateurs (un numéro derrière ce 
qu'il devrait être). Le module détecteur de conflits est peut contenir un dictionnaire 
des procédures à suivre en fonction des événements qu'il déclare en conflit en de 
levant une exception d'après cette situation. L'algorithme \ref{algo:conflictregister} 
présente la fonction utilisée pour détecter des conflits entre plusieurs types 
d'événements. Le registre contient la liste des événements avec lequel un 
événement peut provoquer un conflit. La fonction permet de savoir si un conflit est 
