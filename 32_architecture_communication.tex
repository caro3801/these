...
n\oe uds interconnectés qui ont des fonctions similaires et exécutent des tâches 
...
d'intergiciel comme le routage et la localisation, l'anonymat et la confidentialité.
...
Avec \cite{Desprat2016} la mise en place du modèle événement impose 
...
Enfin, \cite{Desprat2017} va plus loin en distinguant deux rôles parmi les pairs : la 
...
\begin{itemize}
	\item couche de routage et d'échange de messages ;
	\item recherche et stockage de contenu ;
	\item ainsi que configuration et sélection du rôle du pair.
\end{itemize}
...
\begin{description}
	...
	dissémination de l'information. Le partie serveur 
	centralise les données liées à la persistance long  terme;
	décentralise la réplication des données à court terme. Cela évite donc au 
	serveur 
	...
	panne, coupure internet) signifiant la terminaison de la session collaborative.
