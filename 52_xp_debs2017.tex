...
\subsection{Présentation de l'expérimentation 2}
 Cette expérimentation a été réalisée sur la base des contributions liées à 
...
collaborateurs. Afin de permettre la manipulation des objets \gls{3D} facile à 
...
3D). Les participants étaient autorisés à communiquer entre eux.
...
pendant la session.\todo{mettre ailleurs?}
...
\begin{figure}[]
	...
	\\
	\subfloat[Par type 
	d'évènement]{\includegraphics[width=\columnwidth]{eps/byevents.eps}\label{petitb}}
...
le même temps (10-15 minutes). La facilité d'utilisation du système est bien notée 
mais indique encore quelques aspects à améliorer. Pour modérer ces résultat, il 
est important de rappeler que bien que la 
...
manipulant un média (3D) inhabituel pour ce genre de plateforme. De plus, on 
peut également supposer que le prototype créé pour l'expérimentation correspond 
bien au à l'objectif d'assemblage coopératif d'objets \gls{3D} vu que les tâches ont 
...
ont positivement apprécié faire de la modélisation collaborative \gls{3D} dans un 
...
\ref{sec:use_case}, extraite de \cite{Desprat2017}, utilise un prototype issu de 
\cite{Desprat2016} (voir Section \todo{ref proto})reposant sur un modèle orignal 
orienté événements pour la modélisation \gls{3D} collaborative. En menant 
plusieurs 
épreuve au sein de la collaboration sur différents modèles, propose un large panel 
de situations liées au cas d'étude qui engagent leurs capacités de coordination et 
de coopération dans la réalisation.
...
Globalement, les utilisateurs ont exprimé leur satisfaction sur leur expérience 
générale : objectifs atteint, interface facile à prendre en main, collaboration 
facilitant la réalisation de la tâche. Concernant le côté réseau pour la collaboration, 
ils ont pointé la bonne récupération du système dans les cas de 
désynchronisation. Ces derniers n'apparaissent que lors d'une sélection 
multi-utilisateur du même agrégat sont plutôt rares. De plus, les utilisateurs sont 
sensibilisé au fait qu'un autre utilisateur a déjà sélectionné l'objet, les utilisateurs 
concernés sont donc prévenu d'éventuels conflits. 
