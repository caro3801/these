...
dont le contenu est généré : en direct (\textit{live} ou \textit{push}) et à la demande 
(\textit{on-demand} ou \textit{pull}).  

Les requêtes sur le contenu que les n\oe uds doivent récupéré sont effectué selon 
un ordre de priorité déterminé par l'intérêt qu'ils portent au contenu. Par exemple, 
